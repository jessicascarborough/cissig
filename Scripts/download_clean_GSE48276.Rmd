---
title: "Download and clean GSE48276"
...
Now we can see that there are duplicate entries for some genes. For these genes, we'll take the median of each entry. 
...
phenotypes_tidy <- phenotypes %>%
  rename(received_nac = `nac:ch1`,
         survival_mo = `survival (mo):ch1`, 
         censor_os = `os censor:ch1`,
         censor_dss = `dss censor:ch1`,
         c_stage = `cstage:ch1`,
         p_stage = `pstage:ch1`,
         histology_tur = `histology tur:ch1`,
         histology_cyst = `cystectomy histology:ch1`) %>%
  mutate(survival_mo = as.numeric(survival_mo),
         os = case_when(censor_os == "censored" ~ 0, # if the patient is censored
                        censor_os == "uncensored" ~ 1), # by the end of the trial 
         os_dss = case_when(censor_dss == "censored" ~ 0, # then the event didn't happen
                            censor_dss == "uncensored" ~ 1)) %>% # a.k.a. they survived
  select(title, Sample_ID = geo_accession, received_nac, survival_mo, censor_os, 
         censor_dss, os, os_dss, p_stage, c_stage, histology_tur, histology_cyst)
...
Let's combine our phenotypes and gene espression into a single tidied dataset.
